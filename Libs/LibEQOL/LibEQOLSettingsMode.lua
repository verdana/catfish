...
function lib:CreateInput(cat, data)
	...
	local initializer = Settings.CreateControlInitializer(
		"LibEQOL88307f1_InputControlTemplate",
		setting,
		nil,
		data.desc
	)
...
function lib:CreateScrollDropdown(cat, data)
	...
	local initializer = Settings.CreateElementInitializer("LibEQOL88307f1_ScrollDropdownTemplate", {
...
function lib:CreateSoundDropdown(cat, data)
	...
	local setting = registerSetting(
		...
		local initializer = Settings.CreateElementInitializer("LibEQOL88307f1_SoundDropdownTemplate", {
			setting = setting,
...
function lib:CreateColorOverrides(cat, data)
	...
	local initializer = Settings.CreateElementInitializer("LibEQOL88307f1_ColorOverridesPanelNoHead", {
...
function lib:CreateMultiDropdown(cat, data)
	...
	local initializer = Settings.CreateElementInitializer("LibEQOL88307f1_MultiDropdownTemplate", {
		label = data.name or data.text or data.key,
		options = data.values,
		optionfunc = data.optionfunc,
		order = data.order,
...
function lib:CreateText(cat, text, extra)
	...
	local init = Settings.CreateElementInitializer(
		"LibEQOL88307f1_SettingsListSectionHintTemplate",
		{ name = name }
	)
	addSearchTags(init, data.searchtags or name, name)
	applyParentInitializer(init, data.parent, data.parentCheck)
